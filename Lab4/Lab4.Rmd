...
```{r}
model_logit <- glm(vote_for ~ age + isolation + political_view,
                   train,
...
```{r}
model_probit <- glm(vote_for ~ age + isolation + political_view,
                   train,
...
```{r}
model_gompit <- glm(vote_for ~ age + isolation + political_view,
                   train,
...
R2_probit <- PseudoR2(model_logit, which = "McFadden")
R2_gompit <- PseudoR2(model_logit, which = "McFadden")
cat("McFadden R-sq (logit): ", R2_logit, "\n")
...
Т.к. p-value > 0.05, можно сказать, что полученные модели плохо описывают реальные данные.
